...
class Datos extends Conexion{
    public static function obtenerServiciosOS($num_orden, $estado_partida_os){
        $stmt = Conexion::conectar()->prepare("SELECT id_partida_os, codigo_partida_os, descripcion_serv, comentarios_os, 
                            observaciones_os, id_dpto_serv FROM partidas_os 
...
foreach($respuesta as $row => $item){
    echo "<option id='". $item['id_partida_os']."' value='". $item['id_partida_os']."'  data-observaciones='". $item['observaciones_os']."' data-comentarios='". $item['comentarios_os']."' data-dptoServ='".$item['id_dpto_serv']."' >".$item['codigo_partida_os']. " " .$item['descripcion_serv']."</option>";
}
